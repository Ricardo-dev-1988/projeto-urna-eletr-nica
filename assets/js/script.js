    ...
    if(candidato.length > 0){
        ...
        for(let i in candidato.fotos){
            if(candidato.fotos[i].small){
                fotosHtml += `<div class="divisao-1--image small"><img src="assets/images/${candidato.fotos[i].url}" alt=""/>${candidato.fotos[i].legenda}</div>`;
            } else {
                fotosHtml += `<div class="divisao-1--image"><img src="assets/images/${candidato.fotos[i].url}" alt=""/>${candidato.fotos[i].legenda}</div>`;
            }
        }
...
function branco(){
    if(numero === '') {
        ...
        alert('PARA VOTOS EM BRANCO, NÃO DIGITE NENHUM NÚMERO!');
    }
};
function corrige(){
    comecarEtapa();
